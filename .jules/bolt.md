...
## 2023-10-27 - Extract expensive lookups outside loops in React render
**Learning:** Found O(N^2) behavior in `TyreLapPredictor.tsx` where `.find()` was being called multiple times inside an `Array.from()` map on every single live simulation tick, causing severe rendering overhead.
**Action:** When calculating arrays using `.map` or `Array.from` for chart data inside a React `useMemo`, never put `.find()` lookups inside the loop if the array is large or the loop executes frequently. Instead, pre-compute O(1) hash maps (using `forEach` or `reduce`) before the loop to replace the nested O(N) searches.

...
export default function TyreLapPredictor({ season, subTab = "predictor" }: { season: number; subTab?: "predictor" | "circuits" | "accuracy" }) {
  ...
  const chartData = useMemo(() => {
    ...
    // O(1) map pre-computation to avoid O(N^2) `.find()` inside the render loop
    ...
    const simLapMap = new Map(simulatedLaps.map(sl => [sl.lap, sl]));
    ...
    return Array.from({ length: 25 }, (_, idx) => {
      ...
      const simLap = simLapMap.get(lap);

...
class PitStopStrategy:
    ...
    def recommend_pit_window(
        ...
        # Pre-compute lap times for staying out on old compound to avoid O(N²) calculation inside the loop
        max_remaining_laps = total_laps - current_lap
        precomputed_old_times = [
            self.lap_predictor.predict(stint_laps + i, track_temp, max(0.0, fuel_load - (i * 1.55)), current_compound)
            for i in range(max_remaining_laps)
        ]

        for p in range(min_search_lap, max_search_lap + 1):
            laps_on_old = p - current_lap
            
            # Predict times for remaining stint laps on old compound (using precomputed times)
            ...
            # Estimate baseline: stay out on old compound (using precomputed times)
